...
$allowed = array('txt');
  ...
  if (in_array($ext, $allowed)) {
      ...
      $maxq=$q;
      for($i=0;$i<strlen($pass);$i++){
          $q=($q*($pass[$i]) + ($pass[$i]) )%$minint; 
          $maxq=max($q,$maxq);
      }
      $q=gmp_nextprime($maxq);
      ...
      $maxr=$r;
      for($i=0;$i<strlen($pass);$i++){
          $r=($r*($pass[$i])*($pass[$i]))%$minint; 
          $maxr=max($r,$maxr);
      }
      $r=gmp_nextprime($maxr);
      ...
      $maxs=$s;
      for($i=0;$i<strlen($pass);$i++){
          $s=($s*($pass[$i])*($pass[$i]) + ($pass[$i]))%$minint; 
          $maxs=max($s,$maxs);
      }
      $s=gmp_nextprime($maxs);
      echo $s." =s  ";
       echo"<br>";
